...
fn install_ca_certificate(app_handle: tauri::AppHandle) -> Result<(), String> {
    ...
    #[cfg(target_os = "windows")] {
        let output = Command::new("certutil").args(["-addstore", "-f", "ROOT", cert_path_str]).output().map_err(|e| format!("certutil komutu çalıştırılamadı: {}", e))?;
        if !output.status.success() { return Err(format!("certutil başarısız oldu: {}", String::from_utf8_lossy(&output.stderr))); }
    }
    #[cfg(target_os = "macos")] {
        let output = Command::new("sudo").args(["security", "add-trusted-cert", "-d", "-r", "trustRoot", "-k", "/Library/Keychains/System.keychain", cert_path_str,]).output().map_err(|e| format!("security komutu çalıştırılamadı: {}", e))?;
        if !output.status.success() { return Err(format!("security başarısız oldu: {}", String::from_utf8_lossy(&output.stderr))); }
    }
    #[cfg(target_os = "linux")] {
        let cert_dest = "/usr/local/share/ca-certificates/sentiric-ca.crt";
        let copy_output = Command::new("sudo").args(["cp", cert_path_str, cert_dest]).output().map_err(|e| format!("cp komutu çalıştırılamadı: {}", e))?;
        if !copy_output.status.success() { return Err(format!("Sertifika kopyalanamadı: {}", String::from_utf8_lossy(&copy_output.stderr))); }
        let update_output = Command::new("sudo").arg("update-ca-certificates").output().map_err(|e| format!("update-ca-certificates komutu çalıştırılamadı: {}", e))?;
        if !update_output.status.success() { return Err(format!("update-ca-certificates başarısız oldu: {}", String::from_utf8_lossy(&update_output.stderr))); }
    }
...
fn enable_system_proxy() -> Result<(), String> {
    ...
    #[cfg(target_os = "windows")] {
        ...
        Command::new("reg").args(["add", "HKCU\\Software\\Microsoft\\Windows\\CurrentVersion\\Internet Settings", "/v", "ProxyServer", "/d", PROXY_SERVER, "/f"]).output().map_err(|e| format!("ProxyServer ayarı yapılamadı: {}", e))?;
    }
    #[cfg(target_os = "macos")] {
        ...
        .on_system_tray_event(|app, event| {
            if let SystemTrayEvent::MenuItemClick { id, .. } = event {
                match id.as_str() {
                    ...
                    "show" => { if let Some(window) = app.get_window("main") { window.show().unwrap(); window.set_focus().unwrap(); } }
                    _ => {}
                }
            }
        })
